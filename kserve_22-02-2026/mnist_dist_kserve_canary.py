...
def model_serving(model_trained: Input[Model],
    ...
    try:
        ...
        if is_promotion or traffic_split == 100:
            print("PROMOTE: Overwriting stable model and removing canary routing.")
            # We build a clean spec without any canary annotations/splits
            isvc = V1beta1InferenceService(
                api_version='serving.kserve.io/v1beta1',
                kind="InferenceService",
                metadata=client.V1ObjectMeta(
            ...
            # 'replace' forces the ISVC to exactly match this spec (deleting the canary split)
            KServe.replace(name, isvc, namespace=namespace)
            
        else:
            print(f"CANARY: Patching traffic split to {traffic_split}% for new model version.")
            # We use patch to maintain the existing stable version while adding the new one as a canary
            isvc = V1beta1InferenceService(
                api_version='serving.kserve.io/v1beta1',
                kind="InferenceService",
            ...
            KServe.patch(name, isvc, namespace=namespace, canary_traffic_percent=traffic_split)

    except RuntimeError:
        print("INITIAL: No existing service found. Creating fresh deployment.")
        isvc = V1beta1InferenceService(
            api_version='serving.kserve.io/v1beta1',
            kind="InferenceService",
            metadata=client.V1ObjectMeta(
