...
from prometheus_client import Counter # Import this

# Define a custom counter
DIGIT_COUNTER = Counter('mnist_digit_predicted_total', 'Count of digits predicted', ['digit'])
#fetch past metrics --> perform calculations/aggregations" --> add it as customem metric
...
class MnistModel(Model):
    ...
    def predict(self, payload: dict, headers: dict = None) -> dict:
        try:
            ...
            DIGIT_COUNTER.labels(sdigit=str(predicted_digit)).inc()
